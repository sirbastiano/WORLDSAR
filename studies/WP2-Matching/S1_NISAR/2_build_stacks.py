#!/usr/bin/env python3
"""
Pipeline step 2: build NISAR temporal stacks from NISAR-S1 matches.
...
def extract_unique_nisar(matches):
    cols = [
        "nisar_scene_name",
        "nisar_file_id",
        "nisar_start_time",
    ...
    df = matches[cols].drop_duplicates().reset_index(drop=True)
    df["nisar_id"] = df["nisar_scene_name"].fillna(df["nisar_file_id"])
    return df[["nisar_id", "nisar_scene_name", "nisar_file_id", "nisar_start_time", "nisar_wkt"]]


def wkt_to_geom(wkt_value):
    if wkt_value is None or pd.isna(wkt_value):
        return None
    if hasattr(wkt_value, "geom_type"):
        return wkt_value
    return wkt.loads(wkt_value)
...
def build_graph(nisar_df, overlap_threshold):
    footprints = nisar_df["nisar_wkt"].apply(wkt_to_geom)
    G = nx.Graph()
    ids = nisar_df["nisar_id"].tolist()
    for nisar_id in ids:
        G.add_node(nisar_id)
    n = len(ids)
    print(f"Processing {n} NISAR products for spatial overlap")
    for i in range(n):
...
def find_temporal_stacks(nisar_df, graph):
    stacks = list(nx.connected_components(graph))
    temporal_stacks = []
    for stack in stacks:
        stack_df = nisar_df[nisar_df["nisar_id"].isin(stack)].copy()
        stack_df["nisar_start_time"] = pd.to_datetime(stack_df["nisar_start_time"], errors="coerce")
        stack_df = stack_df.sort_values("nisar_start_time")
        temporal_stacks.append(
            stack_df[[
                "nisar_id",
                "nisar_scene_name",
                "nisar_file_id",
...
def summarize_stacks(temporal_stacks, min_products, min_days):
    ...
    for idx, stack in enumerate(temporal_stacks, start=1):
        if len(stack) < min_products:
            continue
        start = stack["nisar_start_time"].min()
        end = stack["nisar_start_time"].max()
...
def write_documentation(output_dir, summary_df, total_stacks, overlap_threshold, min_products, min_days):
    md_content = f"""# NISAR Temporal Stacks Analysis

## Overview
This document describes the NISAR temporal stacks that cover at least {min_days} days
...
## Methodology
- Stacks are derived from NISAR products with >= {overlap_threshold:.2f} spatial overlap.
- Coverage is calculated as the difference between the earliest and latest acquisition dates.
"""
    md_path = output_dir / "nisar_stacks_documentation.md"
...
def main():
    ...
    default_matches = base_dir / "deliverable" / "nisar_s1_IW_matches.parquet"
    default_output = base_dir / "output"
    default_deliverable = base_dir / "deliverable"

    parser = argparse.ArgumentParser(
        description="Pipeline step 2: build NISAR temporal stacks from NISAR-S1 matches"
    )
    parser.add_argument(
        "--matches-path",
        type=str,
        default=str(default_matches),
        help="Path to NISAR-S1 matches parquet",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.6,
        help="Minimum overlap ratio (intersection / min footprint area) to connect products",
    )
    parser.add_argument(
        "--min-products",
        type=int,
    ...
    nisar_df = extract_unique_nisar(matches)
    print(f"Unique NISAR products: {len(nisar_df)}")

    graph = build_graph(nisar_df, args.overlap_threshold)
    temporal_stacks = find_temporal_stacks(nisar_df, graph)
    ...
    parquet_path = deliverable_dir / "nisar_stacks_one_year.parquet"
    csv_path = deliverable_dir / "nisar_stacks_one_year.csv"
